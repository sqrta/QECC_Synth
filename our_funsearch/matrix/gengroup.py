...
def search_2GBAcode(l, m, countA, countB, kthres = 4):
    root = 'group/'
    foldname = root+f'code_{l}_{m}_count{countA}{countB}'
    ...
    for i in range(ri, len(Aterms)):
        ...
        for j in range(jstart, len(Bterms)):
            ...
            k, d = Get_kd_BBCode(gap, A, B, l, m)
            content = f"{'+'.join([PowStr(a1) for a1 in aterm])}, {'+'.join([PowStr(a1) for a1 in bterm])}, {l}, {m}, {l*m}\n"
            if (k,d) in exist.keys():
                exist[(k,d)].append(content)
            else:
                exist[(k,d)]=[content]
            ...
            if iter_count % 10000 ==1:
                for key, value in exist.items():
                    k,d = key
                    filename = foldname + f'/code_{k}_{d}.txt'
